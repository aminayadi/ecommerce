...
type RestOf<T extends IMessage | NewMessage> = Omit<T, 'createdat' | 'hiddenat'> & {
...
export class MessageService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/messages', 'messagedb');
  ...
  protected convertDateFromClient<T extends IMessage | NewMessage | PartialUpdateMessage>(message: T): RestOf<T> {
    return {
      ...message,
      createdat: message.createdat?.format(DATE_FORMAT) ?? null,
      hiddenat: message.hiddenat?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMessage: RestMessage): IMessage {
    return {
      ...
      hiddenat: restMessage.hiddenat ? dayjs(restMessage.hiddenat) : undefined,
    };
  }
