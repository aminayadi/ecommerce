...
type RestOf<T extends IRequest | NewRequest> = Omit<T, 'createdat' | 'modifiedat' | 'deletedat'> & {
...
export class RequestService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/requests', 'requestdb');
  ...
  protected convertDateFromClient<T extends IRequest | NewRequest | PartialUpdateRequest>(request: T): RestOf<T> {
    return {
      ...request,
      createdat: request.createdat?.format(DATE_FORMAT) ?? null,
      modifiedat: request.modifiedat?.format(DATE_FORMAT) ?? null,
      deletedat: request.deletedat?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restRequest: RestRequest): IRequest {
    return {
      ...restRequest,
      createdat: restRequest.createdat ? dayjs(restRequest.createdat) : undefined,
      modifiedat: restRequest.modifiedat ? dayjs(restRequest.modifiedat) : undefined,
      deletedat: restRequest.deletedat ? dayjs(restRequest.deletedat) : undefined,
    };
  }
